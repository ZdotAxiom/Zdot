...
All experiments are fully automated from raw model outputs to the final tables. We provide code and scripts (see README\_REVIEW) to run the paired-generation pipeline and compute the statistics. Raw JSONL logs (including detector flags) are saved for each run; the summary and table-generation scripts are also provided. The sensitive calibration file is not public, but its schema and usage are documented. We release derived artifacts and hashes. Key files:
\path{exp_vnext_ollama_gemma7b_512/summary_5000_sensitive_real/runs_summary_all.csv}\newline
\path{exp_vnext_ollama_gemma7b_512/summary_5000_sensitive_real/counts_by_condition.csv}\newline
\path{exp_vnext_ollama_gemma7b_512/summary_5000_sensitive_real/table1_gemma7b_512_sensitive.csv}.
