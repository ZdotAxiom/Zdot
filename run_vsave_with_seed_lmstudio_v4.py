...
def main():
    ...
    ap.add_argument("--max_interventions", type=int, default=3)
    ...
    args = ap.parse_args()
    # HOTFIX: define interventions before any EMPTYGEN rescue logic uses it
    interventions = 0
    H_pre = None  # HOTFIX: first intervention step index
    ...
    for step in range(int(args.max_new_tokens)):
        ...
        if len(delta) == 0:
            ...
            # FILL NUMERIC METRICS FOR EMPTY_GENERATION (NaN禁止)
            pi_raw = 0.0
            pi_norm = 1.0
            pi_flag = 1
            rep = 0.0
            ...
            too_short = 1
            collapse_flag = 1
            # PATCH(v4): if EMPTY_GENERATION happens in controlled, try intervention instead of immediate collapse
            if args.mode == 'controlled' and (interventions < int(args.max_interventions)):
                intervened = 1
                intervention_type = 'RESET_ON_EMPTYGEN'
                interventions += 1
                if H_pre is None:
                    H_pre = int(step)
                # reset streak and continue generation loop
                consec = 0
                collapse_flag = 0
                collapse_run = 0
                collapse_reason = 'NONE'
            ...
            # PATCH(v3): intervention trigger should follow ANY collapse (rep/too_short/pi)
            # This prevents 'pi_flag-only' no-intervention in controlled.
            pi_flag = int(collapse_flag)
            
        # FINALIZE_INTERVENTION_LOG
        rows.append({
            "record_type": "step",
            "step_idx": int(step),
        ...
        # --- LM sanity: force intervention BEFORE declaring collapse_run ---
        intervened = 0
        intervention_type = "NONE"
        if args.mode == "controlled" and consec >= int(args.k_consecutive) and interventions < int(args.max_interventions):
            interventions += 1
            intervened = 1
            intervention_type = "FORCED_INTERVENE_LM"
            if H_pre is None:
                H_pre = int(step)
            # reset streak so we do not instantly collapse
            consec = 0
            collapse_flag = 0
