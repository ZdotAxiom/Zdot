...
def main():
    ...
    # too-short rule (NeurIPS-friendly)
    # NOTE: min_generated_tokens (not total tokens) avoids prompt-length confounds.
    ...
    meta = {
        ...
        # collapse_flag spec (mode-parity)
    ...
    for step in range(int(args.max_new_tokens)):
        ...
        # baseline collapse rule: sustained PI OR sustained REP
        ...
        # controlled intervention rule: sustained PI triggers deterministic intervention
    ...
    # TOO_SHORT rule (after generation ends)
    final_total_len = int(input_ids.shape[1])
    final_gen_len = max(0, final_total_len - start_len)

    if collapse_run == 0 and final_gen_len < int(min_generated_tokens):
        collapse_run = 1
        collapse_by_short = 1
    ...
    final = {
        ...
        "n_generated_tokens": int(final_gen_len),
