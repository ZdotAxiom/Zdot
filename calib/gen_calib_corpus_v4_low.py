...
# -*- coding: utf-8 -*-
"""
Generate an "independent" calibration corpus (v4 low) tuned to keep pi_raw in a
lower range for GPT-2 runs (avoid code/JSON heavy patterns).

Design:
- Mostly Japanese natural prose (very low English)
- Prefer shorter, single-sentence lines
...
def make_line(rng: random.Random, idx: int) -> str:
    # language mix ratio (mostly Japanese)
    use_ja = rng.random() < 0.90
    if use_ja:
    ...
    if rng.random() < 0.10:
        if use_ja:
            ...
            tail = rng.choice([
                "{a}は過信しない。{b}を見て{c}で確かめる。",
                "{a}より{b}を優先し、{c}は後で。",
                "{a}を一回だけやり、{b}を追加し、{c}は後回し。",
            ]).format(a=a2,b=b2,c=c2)
        else:
            a2,b2,c2 = choose3(rng, EN_ATOMS)
            tail = rng.choice([
                "I avoid overfitting: check {a}, then validate {b} with {c}.",
                "I keep it simple: fix {a}, measure {b}, and note {c}.",
